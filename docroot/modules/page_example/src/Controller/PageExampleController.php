...
class PageExampleController extends ControllerBase {
  ...
  public function simple() {
    $this->loggerFactory->get('page_example')->notice('Simple Page Loaded.');
    $message = 'Simple page: The quick brown fox jumps over the lazy dog.';
    $dispatcher = \Drupal::service('event_dispatcher');

    $e = new DemoEvent($message);
    $event = $dispatcher->dispatch('custom_message_display', $e);

    $new_message = $event->getMessage();

    return array(
      '#markup' => '<p>' . $this->t($new_message) . '</p>',
    );
  }
